async function handleMembershipChange() {
  const selectedMembership =
    document.getElementById("selectedMembership").value;
    ...
    try {
        ...
        if (window.currentUser && window.currentUser.username === "Admin") {
          adminButton = `
              <select data-userId ="${item.userid}" class= "membership-select" >
                  <option${
                    item.membershipName === "Bronze" ? "selected" : ""
                  }>Bronze</option>
                  <option${
                    item.membershipName === "Silver" ? "selected" : ""
                  }>Silver</option>
                  <option${
                    item.membershipName === "Gold" ? "selected" : ""
                  }>Gold</option>
              </select>
              <button class="change-membership-btn" data-userId="${
                item.userid
              }">Change</button>
               `;
        }
      ...
      tbody.querySelectorAll(".change-membership-btn").forEach((btn) => {
        btn.addEventListener("click", async (e) => {
          ...
          try {
            const response = await fetch(`/api/orders/${userId}/membership`, {
              method: "POST",
              credentials: "include",
