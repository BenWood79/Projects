...
async function openProductModal(product){
    ...
    document.getElementById('modalProductDateAdded').value = product.date_added ? product.date_added.slice(0, 10) : '';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (editForm) {
    editForm.addEventListener('submit', async function(event) {
      ...
      try{
        const response = await fetch(`http://localhost:3002/products/${currentProduct}`, {
          method: 'PUT',
          credentials: 'include',
          headers: {
