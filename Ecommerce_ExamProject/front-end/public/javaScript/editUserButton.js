  ...
  if (editForm) {
    editForm.addEventListener('submit', async function(event) {
      ...
      const updatedUser = {
        ...
        try{
        const response = await fetch(`http://localhost:3002/users/${currentUser}`, {
            method: 'PUT',
            credentials: 'include',
            headers: {
        ...
        tbody.innerHTML = '';
          ...
          for (const item of users) {         
            ...
            tr.innerHTML = `
              ...
              <td class="short-date">${users.createdAt ? users.createdAt.slice(0, 10).split('-').reverse().join('/') : ''}</td>
              <td class="short-date">${users.updatedAt ? users.updatedAt.slice(0, 10).split('-').reverse().join('/') : ''}</td>
              <td>
                  <button class="btn btn-warning btn-sm mx-1"
                    onclick="deleteUser('/users', '${users.id}')">Delete</button>
                  <button class="btn btn-danger btn-sm mx-1"
                    onclick="openUserModal(JSON.parse(decodeURIComponent('${encodeURIComponent(JSON.stringify(users))}')))">
                    Edit
                  </button>
              </td>
            `;
