...
const InsertData = async (filename) => {
  const { records } = await JSON.parse(
    fs.readFileSync("./public/json/" + filename)
  );
  console.log(records);
  for (const record of records) {
    try {
      ...
      if (error.name === "SequelizeUniqueConstraintError") {
        console.log(
          `Skipping duplicate entry: ${error.fields ? JSON.stringify(error.fields) : "unknown"}`
        );
      } else {
...
async function CheckIfDBHasData() {
  try {
    ...
    let species = await db.sequelize.query(
    ...
    let temperaments = await db.sequelize.query(
      "SELECT COUNT(*) as total FROM temperaments",
      {
        raw: true,
        type: QueryTypes.SELECT,
      }
